...
class Zipmoney_ZipmoneyPayment_Helper_Payload extends Zipmoney_ZipmoneyPayment_Helper_Abstract
{
    ...
    public function getCheckoutPayload($quote)
    {
        ...
        return $checkoutReq->setType("standard")->setShopper($this->getShopper())->setOrder($this->getOrderDetails(new CheckoutOrder))->setMetadata($this->getMetadata())->setConfig($this->getCheckoutConfiguration());
    }
    ...
    public function getChargePayload($order)
    {
        ...
        return $chargeReq->setAmount((float) $grand_total)->setCurrency($currency)->setOrder($this->getOrderDetails(new ChargeOrder))->setMetadata($this->getMetadata())->setCapture($this->_config->isCharge())->setAuthority($this->getAuthority());
    }
    ...
    public function getRefundPayload($order, $amount, $reason)
    {
        ...
        return $chargeReq->setAmount((float) $amount)->setReason($reason)->setChargeId($order->getPayment()->getZipmoneyChargeId())->setMetadata($this->getMetadata());
    }
    ...
    public function getOrderDetails($reqOrder)
    {
        ...
        $reqOrder->setReference($reference)->setShipping($this->getShippingDetails())->setItems($orderItems);
        return $reqOrder;
    }
    ...
    public function getOrderItems()
    {
        ...
        foreach ($items as $item) {
            ...
            $orderItem->setName($item->getName())->setAmount($item->getPriceInclTax() ? (float) $item->getPriceInclTax() : 0.00)->setReference((string) $item->getId())->setDescription($description)->setQuantity(round($qty))->setType("sku")->setImageUri($this->_getProductImage($item))->setItemUri($item->getProduct()->getProductUrl())->setProductCode($item->getSku());
    ...
    public function getAuthority()
    {
        ...
        $authority->setType('checkout_id')->setValue($checkout_id);
    ...
    public function getOrderOrQuoteCustomer($shopper, $order_or_quote)
    {
        ...
        $shopper->setFirstName($order_or_quote->getCustomerFirstname())->setLastName($order_or_quote->getCustomerLastname())->setEmail($order_or_quote->getCustomerEmail());
    ...
    public function getCustomer($shopper, $customer)
    {
        ...
        if (Mage::helper('customer')->isLoggedIn() || $customer->getId()) {
            ...
            $orderCollection = Mage::getModel('sales/order')->getCollection()->addFieldToFilter('customer_id', array('eq' => array($customer->getId())))->addFieldToFilter(
                'state', array(
            ...
            $statistics->setAccountCreated($customer->getCreatedAt())->setSalesTotalCount((int) $orderNum)->setSalesTotalAmount((float) $lifetimeSalesAmount)->setSalesAvgAmount((float) $averageSaleValue)->setSalesMaxAmount((float) $maximumSaleValue)->setRefundsTotalAmount((float) $lifetimeSalesRefundedAmount)->setPreviousChargeback($chargeBackBefore)->setCurrency(Mage::app()->getStore()->getCurrentCurrencyCode());
    ...
    {
        $genderText = Mage::getModel('customer/customer')->getResource()->getAttribute('gender')->getSource()->getOptionText($gender);
        return $genderText;
    }
