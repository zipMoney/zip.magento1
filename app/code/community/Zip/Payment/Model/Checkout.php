...
class Zip_Payment_Model_Checkout
{
    ...
    {
        $hasCart = $this->getQuote()->getId() && $this->getQuote()->getIsActive();
        $response = array(
            'success' => false,
            'message' => null,
            'redirect_url' => $hasCart ? Zip_Payment_Model_Config::
            CHECKOUT_CART_URL_ROUTE : Zip_Payment_Model_Config::
            CHECKOUT_FAILURE_URL_ROUTE
        );

        try {
            $checkoutIdFromSession = $this->getHelper()->getCheckoutIdFromSession();
            $isReferred = !empty($checkoutId) &&
            (empty($checkoutIdFromSession) || $checkoutIdFromSession != $checkoutId);
    ...
    protected function placeOrderWithExistingOrder($order)
    {
        ...
        if (!$this->getHelper()->isReferredOrder($order)) {
            Mage::throwException(
                $this->getHelper()->__('Current order is not a valid referred order. payment can\'t be processed.')
            );
        }
        ...
        if (!$order->canInvoice()) {
            Mage::throwException(
                $this->getHelper()->__('Cannot create an invoice for this order.')
            );
        }
        ...
        try {
            ...
            $invoice
                ->addComment('Invoice generated by Zip Payment - Referred Application')
                ->register()
                ->sendEmail(true);
