...
class Zip_Payment_Model_Checkout
{
    ...
    public function handleResponse($checkoutId, $state) {
        ...
        try {
            ...
            $checkoutId = empty($checkoutIdFromSession) ?: $checkoutIdFromSession;
            ...
            switch($state) {
                case Zip_Payment_Model_Api_Checkout::STATE_APPROVED: 
                    ...
                    $isReferred ? $this->placeReferredOrder() : $this->placeOrder();
    ...
    protected function placeOrderWithExistingOrder($order) {
        ...
        try {
            ...
            $invoice
            ->addComment('Invoice generated automatically')
            ->register()
            ->sendEmail(true);
    ...
    protected function redirectAfterResponse($response) {
        ...
        $url = Mage::getUrl($response['redirect_url'], array('_secure' => true));

        // if it's an ajax call
        if(Mage::app()->getRequest()->isAjax()) {
            $response['redirect_url'] = Mage::getUrl($url);
